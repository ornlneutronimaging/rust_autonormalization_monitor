#!/usr/bin/env bash
# Launch the Auto Normalization Monitor GUI, rebuilding first if the sources
# changed.
#
# Usage: ./launch_autonormalization_monitor.sh
set -euo pipefail

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BINARY="$REPO_DIR/target/release/autonormalization_monitor"
...
if $needs_build; then
    ...
    echo "Building autonormalization_monitor (release)..."
